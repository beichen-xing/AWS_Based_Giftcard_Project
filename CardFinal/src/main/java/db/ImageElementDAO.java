...
public class ImageElementDAO {
    ...
    private ImageElement generateImage(ResultSet resultSet) throws Exception {
    	...
    	String card_content = resultSet.getString("card_content");
    	String image_id_unique = resultSet.getString("image_id_unique");
       
        return new ImageElement(image_id, image_path, bounds, page, card_id,card_content, image_id_unique);
    }
	...
	public List<String> getAllImagespath(String id) throws Exception {
		 List<String> allImagesPath = new ArrayList<>();
	        try {
	            Statement statement = conn.createStatement();
	            PreparedStatement ps = conn.prepareStatement("SELECT image_path FROM Images WHERE card_id=?;");
	            ps.setString(1, id);
	            ResultSet resultSet = ps.executeQuery();

	            while (resultSet.next()) {
	                String path = generateImagePath(resultSet);
    ...
    private String generateImagePath(ResultSet resultSet) throws Exception {
    	String image_path  = resultSet.getString("image_path");
        return image_path;
    }
